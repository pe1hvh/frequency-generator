...
// Libraries
#include <Wire.h>                      // Allows the communication between devices or sensors connected via Two Wire Interface Bus. Specific implementation for nRF52. This Library is needed for si5341
#include <Rotary.h>                    // Arduino library for reading rotary encoders that output a 2-bit gray code. Ben  Buxton https://github.com/brianlow/Rotary
...
#define XT_CAL_F   42000     //Si5351 calibration factor, adjust to get exatcly 10MHz. Increasing this value will decreases the frequency and vice versa.
...
Rotary r = Rotary(2, 3);                                       // Initialize Rotary opbject r (Type Rotary) with use of pin 2 and pin 3
Adafruit_SSD1306 display = Adafruit_SSD1306(128, 64, &Wire);   // Initialize Adafruit_SSD1306 object (Type Adafruit_SSD1306) width 128 pixels hight 64 pixels, using the Wire object(library)
...
ISR(PCINT2_vect) {                                  // Interup Service Routine PCINT2_vect: interrupt vector for PORTD
  char result = r.process();
  if (result == DIR_CW) set_frequency(1);           // DIR_CW = Direction Clockwise step.
  else if (result == DIR_CCW) set_frequency(-1);    // DIR_CCW = Direction Counter-clockwise step.
}
...
void setup() {
  ...
  display.begin(SSD1306_SWITCHCAPVCC,  0x3C);     // SSD1306_SWITCHCAPVCC generate display voltage from 3.3V internally. 0x3C is Screen Address.Question 0x3D for 128x64 ?
  display.clearDisplay();
  display.setTextColor(WHITE);
  display.display();

  pinMode(2, INPUT_PULLUP);                       //  The pin 2 is configured as an input, meaning it can read digital signals (HIGH or LOW) and an internal pull-up resistor is enabled for the pin.
  ...
  si5351.init(SI5351_CRYSTAL_LOAD_8PF, 0,0);             // SI5351_CRYSTAL_LOAD_8PF: This parameter sets the crystal load capacitance to 8 picofarads (pF). The Si5351 has an internal crystal load capacitor that can be configured to match the requirements of the external crystal oscillator. Setting it to 8pF helps ensure proper oscillation and frequency stability. 0 = no correction , 0 = no extenal reference clock
  si5351.set_correction(cal, SI5351_PLL_INPUT_XO);       // Parameters  cal: This is the calibration factor, usually an integer value representing parts per billion (ppb) of frequency error. SI5351_PLL_INPUT_XO: This parameter specifies that the correction is being applied to the crystal oscillator input of the PLL (Phase-Locked Loop).
  si5351.drive_strength(SI5351_CLK0,  SI5351_DRIVE_8MA); // SI5351_CLK0 specifies that this setting applies to clock output 0 of the Si5351. SI5351_DRIVE_8MA sets the drive strength to 8 milliamps (mA).
  si5351.output_enable(SI5351_CLK0,1);                   // 1 - Enable / 0 - Disable CLK
  si5351.output_enable(SI5351_CLK1,0);
  si5351.output_enable(SI5351_CLK2, 0);

  PCICR |= (1 << PCIE2);                       // Pin Change Interrup Control Register is set with a Pin Change Enable bit => PCIE2. PCIE2 is responsible for PORTD (Digital pin 1 tot en met 6). pin change interrupts at PORTD are allowed.
  PCMSK2 |= (1 << PCINT18) | (1 << PCINT19);   // Enables the pin change interrupt at PD2 (= Arduino pin 2) and PD3 (=Arduino pin 3) .
  sei();                                       // Activate the interrupts.Now interrupts starts working.https://wolles-elektronikkiste.de/en/interrupts-part-1-external-interrupts & https://wolles-elektronikkiste.de/en/interrupts-part-2-pin-change-interrupts
